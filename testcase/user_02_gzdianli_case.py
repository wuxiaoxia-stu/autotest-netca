...
#用例：贵州电力单位用户自助（如59，https://192.168.0.59/gzdianli_org）
#业务信息：普通申请流程，普通发票、无快递、企业银联支付。上传2个附件。
class GzdianliCase(Base):

    url = Base.ip + "/gzdianli_org"
    ...
    def tearDown(self):
        if self.test == 0 :
            nowTime = self.now
            screenshot("贵州电力"+nowTime)#用例脚本运行报错立刻截图保存
        #self.driver.close()
